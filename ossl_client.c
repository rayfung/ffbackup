...
**	-d <dir>	CA certificate directory
**	-i		Ignore (don't verify) server certificate
**	-v		Verify host name in server certificate
...
main( int argc, char **argv )
{
    ...
    while( (c = getopt( argc, argv, "ac:d:hinp:St:TvV" )) != -1 )
	switch(c)
	{
        case 'h':
	    ...
	    printf( "-d <dir>\tCA certificate directory\n" );
            printf( "-i\t\tIgnore (don't verify) server certificate\n" );
    ...
    if ( tlsv1 && sslv2 )
	err_exit( "-T and -S both defined" );

    if ( cafile || cadir || ! (anonCipher || nullCipher) )
	certificate = 1;
    ...
    if ( ! ignore )
      check_certificate( ssl, ! anonCipher, verify ? host : NULL );
...
static void 
check_certificate( SSL *ssl, int required, char *host )
{
    X509 *peer;
    char peer_CN[ 256 ];
...
static void
client_request( SSL *ssl, char *host, int port )
{
    BIO *io, *ssl_bio;
    char buf[ 1024 ];
    int r, len;
	...
	case SSL_ERROR_ZERO_RETURN:		return;
	case SSL_ERROR_SYSCALL:	ssl_err_exit( "SSL Error: Premature close" );
